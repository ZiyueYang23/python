...
class ChessCover:
    def __init__(self, side_length):
        ...
        self.board = [
            ["" for i in range(self.orig_length)] for i in range(self.orig_length)
        ]
        self.special_point = [
            random.randint(1, self.orig_length - 1),
            random.randint(0, self.orig_length - 1),
        ]

        self.special_point_to_print = [
            self.special_point[0] + 1,
            self.special_point[1] + 1,
        ]
    ...
    def _init_canvas(self):
        ...
        for i in range(self.orig_length):  # 以下为画行
            ...
            for j in [1, -1]:
                goto(
                    j * (self.ZERO - Border / 4),
                    self.ZERO
                    + i * self.each_squar_length
                    + self.each_squar_length * (1 / 3),
                )  # 写行标
                write(str(i + 1), ["", 25])
    ...
    def _paint_upper_vertical(self, _position):
        ...
        pendown()
        goto(
            _position[0] + self.scale * self.each_squar_length,
            _position[1] + (1 - self.scale) * self.each_squar_length,
        )
        goto(
            _position[0] + (1 - self.scale) * self.each_squar_length,
            _position[1] + (1 - self.scale) * self.each_squar_length,
        )
    ...
    def _paint_left_horizontal(self, _position):
        penup()
        goto(
            _position[0] + self.each_squar_length,
            _position[1] + (1 - self.scale) * self.each_squar_length,
        )
        pendown()
        goto(
    ...
    def _paint_special_point(self):
        ...
        goto(
            self.ZERO
            + self.special_point[0] * self.each_squar_length
            + self.scale * self.each_squar_length,
            self.ZERO
            + self.special_point[1] * self.each_squar_length
            + self.scale * self.each_squar_length,
        )
        pendown()
        goto(
            ...
            self.ZERO
            + self.special_point[1] * self.each_squar_length
            + (1 - self.scale) * self.each_squar_length,
        )
        goto(
            self.ZERO
            + self.special_point[0] * self.each_squar_length
    ...
    def chess_cover(self, position, n_length, special_point):
        ...
        if n == 1:
            ...
            A_12 = [
                position[0] + n / 2 * self.each_squar_length,
                position[1] + n / 2 * self.each_squar_length,
            ]
            A_21 = [position[0], position[1]]
            A_22 = [position[0] + n / 2 * self.each_squar_length, position[1]]
            if (
                special_point[0] < position[0] + n / 2 * self.each_squar_length
            ):  # 特殊点位于棋盘左半边
                if (
                    special_point[1] < position[1] + n / 2 * self.each_squar_length
                ):  # 特殊点位于棋盘左下角
                    a_11 = [
                        position[0] + (n / 2 - 1) * self.each_squar_length,
                        position[1] + n / 2 * self.each_squar_length,
                    ]
                    a_12 = [
                        position[0] + n / 2 * self.each_squar_length,
    ...
    def ChessCover(self):
        special_point = [
            self.ZERO + self.special_point[0] * self.each_squar_length,
...
class StartMenu:
    def __init__(self):
        self.screen = Screen()
        self.screen.title("L型骨牌覆盖游戏")
        self.screen.bgcolor("white")
        self.screen.setup(
            width=TurtleWindowHeight + Border, height=TurtleWindowHeight + Border
        )

        self.message = Turtle()
        self.message.hideturtle()
        self.message.penup()
        self.message.goto(0, 100)
        self.message.pendown()
        self.message.write(
            "欢迎来到L型骨牌覆盖游戏!", align="center", font=("Arial", 24, "normal")
        )
        self.message.penup()
        self.message.goto(0, -100)
        self.message.pendown()
        self.message.write(
            "按任意键开始游戏...", align="center", font=("Arial", 18, "normal")
        )
...
def main():
    ...
    start_menu.wait_for_start()  # 显示开始界面并等待用户输入
